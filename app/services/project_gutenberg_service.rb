  ...
  def fetch_text_content(gutenberg_id)
    ...
    text_urls.each do |url|
      ...
      if response.success? && response.body.present?
        return clean_text(response.body)
      end
    end
